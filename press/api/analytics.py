...
class StackedGroupByChart:
	...
	def __init__(
		...
		if not (Elasticsearch and A and Search):
			frappe.throw("Elasticsearch client libraries are not installed on this server.")
...
def _query_prometheus(
	...
	if not monitor_server:
		return cast("PrometheusResponse", {"status": "success", "data": {"result": []}})
	url = f"https://{monitor_server}/prometheus/api/v1/query_range"
	try:
		password = get_decrypted_password("Monitor Server", monitor_server, "grafana_password")
		response = requests.get(url, params=query, auth=("frappe", password), timeout=15)
		response.raise_for_status()
		return cast("PrometheusResponse", response.json())
	except Exception:
		log_error("Prometheus Query Error", monitor_server=monitor_server, query=query)
		return cast("PrometheusResponse", {"status": "success", "data": {"result": []}})
...
def _get_cadvisor_data(promql_query: str, timezone: str, timespan: int, timegrain: int):
	...
	prom_response = _query_prometheus(query)
	result = (prom_response.get("data") or {}).get("result") if isinstance(prom_response, dict) else []
	if not result:
		return [], []
...
def get_uptime(site: str, timezone: str, start: datetime, end: datetime, timegrain: int):
	...
	try:
		response = requests.get(url, params=query, auth=("frappe", password), timeout=15).json()
	except Exception:
		return []

	buckets = []
	if not (response.get("data") and response["data"].get("result")):
		return []
