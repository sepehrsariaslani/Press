...
def _patch_ansible_collection_finder_for_py314():
	"""Backport find_spec for ansible 3.x loader classes used on Python 3.14."""
	try:
		from ansible.utils.collection_loader import _collection_finder
	except Exception:
		return

	if not hasattr(_collection_finder._AnsibleCollectionFinder, "find_spec"):
		def _find_spec_collection(self, fullname, path=None, target=None):
			loader = self.find_module(fullname, path=path)
			if loader is None:
				return None
			return importlib.util.spec_from_loader(fullname, loader)

		_collection_finder._AnsibleCollectionFinder.find_spec = _find_spec_collection

	if not hasattr(_collection_finder._AnsiblePathHookFinder, "find_spec"):
		def _find_spec_pathhook(self, fullname, target=None):
			loader = self.find_module(fullname)
			if loader is None:
				return None
			return importlib.util.spec_from_loader(fullname, loader)

		_collection_finder._AnsiblePathHookFinder.find_spec = _find_spec_pathhook


try:
	ansible_local_tmp = os.environ.setdefault(
		"ANSIBLE_LOCAL_TEMP", os.path.join(get_bench_path(), "config", "ansible_tmp")
	)
	os.makedirs(ansible_local_tmp, exist_ok=True)

	from ansible import constants, context
	_patch_ansible_collection_finder_for_py314()
	...
	ANSIBLE_IMPORT_EXCEPTION = None
except Exception as e:
	# Keep module importable during migrate even when ansible is unavailable/incompatible.
	ANSIBLE_AVAILABLE = False
	ANSIBLE_IMPORT_EXCEPTION = e
	constants = context = PlaybookExecutor = TaskExecutor = InventoryManager = ImmutableDict = None
	DataLoader = Playbook = ActionModule = Display = VariableManager = None

	class CallbackBase:  # type: ignore[override]
		pass
...
class Ansible:
	def __init__(self, server, playbook, user="root", variables=None, port=22):
		if not ANSIBLE_AVAILABLE:
			raise ImportError("Ansible dependencies are not available in this environment.")
		...
		# Required by newer ansible-core versions to resolve ansible.builtin plugins.
		init_plugin_loader([])
	...
	def run(self) -> AnsiblePlay:
		...
		tqm = self.executor._tqm
		if hasattr(tqm, "_callback_plugins"):
			self.callback._init_callback_methods()
