...
try:
	...
	MetricRangeDataFrame = typing.Any
	PrometheusConnect = typing.Any
...
def get_prometheus_client() -> PrometheusConnect:
	...
	if PrometheusConnect is typing.Any:
		frappe.throw("prometheus_api_client library is not installed on this server.")
...
class AppServerInvestigationActions:
	...
	def get_oom_kill_events(self, step: "ActionStep"):
		...
		if Elasticsearch is None:
			step.status = StepStatus.Failure
			step.output = "Unable to fetch OOM kill events because Elasticsearch client is not installed"
			step.save()
			return
