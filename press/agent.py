...
class Agent:
	...
	def _make_req(self, method, path, data, files, agent_job_id):
		...
		if frappe.conf.developer_mode:
			...
			if intermediate_ca:
				...
				# Local developer setups often use self-signed agent certs.
				verify = False
		elif self.server_type in ("Server", "Database Server", "Proxy Server") and frappe.db.get_value(
			self.server_type, self.server, "is_self_hosted"
		):
			# Self-hosted installs often terminate /agent over self-signed TLS.
			# Keep requests functional by skipping strict CA verification in this mode.
			verify = False
	...
	def _get_request_url(self, path):
		...
		if self.server_type in ("Server", "Database Server", "Proxy Server"):
			proxy = None
		...
		return f"https://{target_host}:{self.port}/agent/{path}"
