...
try:
	...
	oci_pagination = None
	BlockstorageClient = ComputeClient = VirtualNetworkClient = None
	CreateBootVolumeBackupDetails = CreateVnicDetails = CreateVolumeBackupDetails = None
	GetPublicIpByIpAddressDetails = InstanceOptions = InstanceSourceViaImageDetails = None
	LaunchInstanceDetails = LaunchInstancePlatformConfig = LaunchInstanceShapeConfigDetails = None
	UpdateBootVolumeDetails = UpdateInstanceDetails = UpdateInstanceShapeConfigDetails = None
	UpdateVolumeDetails = None
	TransientServiceError = Exception
...
class VirtualMachine(Document):
	...
	def client(self, client_type=None):
		...
		if self.cloud_provider == "DigitalOcean":
			if pydo is None:
				frappe.throw("DigitalOcean client library is not installed on this server.")
