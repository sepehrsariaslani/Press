...
class Bench(Document):
	...
	def server_logs(self):
		server_info = frappe.db.get_value(
			"Server",
			self.server,
			["is_self_hosted", "is_server_setup"],
			as_dict=True,
		)
		if server_info and server_info.is_self_hosted and not server_info.is_server_setup:
			return []

		try:
			return Agent(self.server).get(f"benches/{self.name}/logs") or []
		except AgentRequestSkippedException:
			return []
		except Exception:
			log_error("Bench logs fetch failed", bench=self.name, server=self.server)
			return []
	...
	def get_server_log(self, log):
		...
		try:
			return Agent(self.server).get(f"benches/{self.name}/logs/{log}") or {log: ""}
		except AgentRequestSkippedException:
			return {log: ""}
		except Exception:
			log_error("Bench log file fetch failed", bench=self.name, server=self.server, log=log)
			return {log: ""}
	...
	def get_server_log_for_log_browser(self, log):
		...
		try:
			return Agent(self.server).get(f"benches/{self.name}/logs_v2/{log}") or {log: ""}
		except AgentRequestSkippedException:
			return {log: ""}
		except Exception:
			log_error("Bench log browser fetch failed", bench=self.name, server=self.server, log=log)
			return {log: ""}
