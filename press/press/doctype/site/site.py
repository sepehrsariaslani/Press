...
class Site(Document, TagHelpers):
	...
	def server_logs(self):
		...
		try:
			return Agent(self.server).get(f"benches/{self.bench}/sites/{self.name}/logs") or []
		except AgentRequestSkippedException:
			return []
		except Exception:
			log_error("Site logs fetch failed", site=self.name, server=self.server, bench=self.bench)
			return []
	...
	def get_server_log(self, log):
		...
		try:
			return Agent(self.server).get(f"benches/{self.bench}/sites/{self.name}/logs/{log}") or {log: ""}
		except AgentRequestSkippedException:
			return {log: ""}
		except Exception:
			log_error(
				"Site log file fetch failed",
				site=self.name,
				server=self.server,
				bench=self.bench,
				log=log,
	...
	def get_server_log_for_log_browser(self, log):
		...
		try:
			return Agent(self.server).get(f"benches/{self.bench}/sites/{self.name}/logs_v2/{log}") or {log: ""}
		except AgentRequestSkippedException:
			return {log: ""}
		except Exception:
			log_error(
				"Site log browser fetch failed",
				site=self.name,
				server=self.server,
				bench=self.bench,
				log=log,
