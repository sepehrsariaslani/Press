...
# Keep heavy Press infra imports lazy so site sync/migrate can run even when
# optional provider SDKs are not installed yet in shared stacks.
class Site:
	def __new__(cls, *args, **kwargs):
		from press.press.doctype.site.site import Site as _Site

		return _Site(*args, **kwargs)


def process_backup_site_job_update(*args, **kwargs):
	from press.press.doctype.site_backup.site_backup import (
		process_backup_site_job_update as _process_backup_site_job_update,
	)

	return _process_backup_site_job_update(*args, **kwargs)
