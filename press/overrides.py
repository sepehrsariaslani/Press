...
def cleanup_ansible_tmp_files():
	...
	try:
		...
		# Ansible is optional in some setups; skip temp cleanup if unavailable.
		return

	if not hasattr(ansible_constants, "DEFAULT_LOCAL_TMP"):
		return
	...
	temp_dir = pathlib.Path(ansible_constants.DEFAULT_LOCAL_TMP).parent
...
def sync_press_workspace_on_migrate():
	from frappe.modules.import_file import import_file_by_path

	workspace_path = frappe.get_app_path("press", "press", "workspace", "press", "press.json")
