...
export default {
	...
	methods: {
		...
		getPlanFeatures(plan) {
			let features = [
				`${plan.cpu_time_per_day} ساعت پردازش CPU در روز`,
				this.formatBytes(plan.max_database_usage, 0, 2) + ' دیتابیس',
				this.formatBytes(plan.max_storage_usage, 0, 2) + ' فضای ذخیره‌سازی',
			];
			if (plan.support_included) {
				features.push('ضمانت محصول + پشتیبانی');
			}
			return features;
		},
		capturePosthogEvent() {
