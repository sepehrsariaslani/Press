		...
		localizeOption(option) {
			...
			return {
				...
				options: Array.isArray(option.options)
					? option.options.map(this.localizeOption)
					: option.options,
			};
		},
		reloadListView() {
	...
	computed: {
		...
		searchQuerySummary() {
			...
			if (this.filteredRows.length === 0) {
				...
				summary = `${filtered} از ${total}`;
			} else {
				summary = `${this.filteredRows.length} از ${this.rows.length}`;
			}
			return summary;
		},
		filterControls() {
			...
			return controls
				...
				.map((control) => {
					return reactive({
						...control,
						label: tr(control.label),
						placeholder: tr(control.placeholder),
						options: Array.isArray(control.options)
							? control.options.map(this.localizeOption)
							: control.options,
						value: control.default || undefined,
					});
				});
		},
		actions() {
		...
		primaryAction() {
			...
			return this.localizeOption(props);
		},
		secondaryAction() {
		...
		emptyStateMessage() {
			return tr(this.options.emptyStateMessage || 'No results found');
		},
		banner() {
