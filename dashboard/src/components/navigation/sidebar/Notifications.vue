	...
	toast.promise(
		...
		}),
		{
			success: () => {
				...
				return 'همه اعلان‌ها خوانده شدند';
			},
			loading: 'در حال علامت‌گذاری همه اعلان‌ها به‌عنوان خوانده‌شده...',
			error: (error) =>
...
const tabs = [
	{ label: 'همه', icon: LucideRows2 },
	{ label: 'درخواست‌ها', icon: LucideKeySquare },
	{ label: 'خوانده‌نشده', icon: LucideMessageSquareDot },
];
</script>
		...
		<template #body="{ togglePopover }">
			...
			>
				...
				<div class="text-base flex items-center py-2 pl-4 pr-2 border-b">
					<span class="font-medium mr-auto">اعلان‌ها</span>
				...
				<Scrollbar ref="scrollRef" v-if="resource.data.length > 0">
					<div
						...
						title="برای خوانده‌شدن کلیک کنید"
					>
						<!-- type icon -->
						<div
						...
						>
							<div class="flex">
								...
								<Button
									...
									tooltip="اقدامات دسترسی پشتیبانی"
									variant="ghost"
									class="mb-auto"
							...
							<Badge class="text-xs mr-auto">
								...
								<Tooltip
									text="این اعلان نیاز به توجه شما دارد"
									:hoverDelay="0"
