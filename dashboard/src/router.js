...
async function ensureSellerOwnerAccess(to, from, next) {
	...
	try {
		await session.userPermissions.fetch();
	} catch (e) {
		console.warn('Unable to load user permissions for seller dashboard:', e);
	}

	if (session.userPermissions.data.owner || session.isSystemUser) {
		next();
		return;
	}
...
let router = createRouter({
	...
	routes: [
		...
		},
		{
			...
			beforeEnter: ensureSellerOwnerAccess,
			redirect: { name: 'SellerOverview' },
			children: [
				{
					name: 'SellerOverview',
					path: 'overview',
					component: () => import('./pages/seller/SellerOverview.vue'),
				},
				{
					name: 'SellerApps',
					path: 'apps',
					component: () => import('./pages/seller/SellerApps.vue'),
				},
				{
					name: 'SellerSubscriptions',
					path: 'subscriptions',
					component: () => import('./pages/seller/SellerSubscriptions.vue'),
				},
			],
		},
		{
...
router.beforeEach(async (to, from, next) => {
	...
	if (isLoggedIn && hasTeamPrivileges) {
		...
		if (privateSourceOnly && String(to.name || '').startsWith('Marketplace App')) {
			try {
				await session.userPermissions.fetch();
			} catch (e) {
				console.warn('Unable to load user permissions for marketplace access:', e);
			}
			const canManageMarketplace =
				Boolean(session.userPermissions.data.owner) || session.isSystemUser;
