...
export function utcDate(dateTimeString, format = 'LLLL') {
	...
	let localDate = dayjs.utc(dateTimeString).local().format('YYYY-MM-DD HH:mm:ss');
	return formatJalaliDate(localDate, format);
}

export function date(dateTimeString, format = 'LLLL') {
	if (!dateTimeString) return;
	let localDate = dayjsLocal(dateTimeString).format('YYYY-MM-DD HH:mm:ss');
	return formatJalaliDate(localDate, format);
}
...
export function prettyDate(date, mini = false) {
	...
	const rtf = new Intl.RelativeTimeFormat('fa-IR', {
		numeric: 'auto',
		style: mini ? 'narrow' : 'long',
	});

	const deltaSeconds = date.diff(nowDatetime, 'seconds');
	const absoluteDelta = Math.abs(deltaSeconds);

	if (absoluteDelta < 45) {
		return mini ? 'اکنون' : 'همین الان';
	}
	...
	if (absoluteDelta < 3600) {
		...
		unit = 'minute';
	} else if (absoluteDelta < 86400) {
		value = Math.round(deltaSeconds / 3600);
		unit = 'hour';
	} else if (absoluteDelta < 604800) {
		value = Math.round(deltaSeconds / 86400);
		unit = 'day';
	} else if (absoluteDelta < 2629800) {
		value = Math.round(deltaSeconds / 604800);
		unit = 'week';
	} else if (absoluteDelta < 31557600) {
		value = Math.round(deltaSeconds / 2629800);
		unit = 'month';
	} else {
		value = Math.round(deltaSeconds / 31557600);
		unit = 'year';
	}

	return toPersianDigits(rtf.format(value, unit));
}
